...
def buscar_id_business(business_id):
	...
	with conn.cursor() as cur:
		...
		if row:
			...
			business = dict(zip(colnames, row))
		else:
			business = None
	conn.close()
	return business
...
def mostrar_business(search_query='', categoria=''):
	...
	with conn.cursor() as cur:
		...
		rows = cur.fetchall()
		colnames = [desc[0] for desc in cur.description]
		businesses = [dict(zip(colnames, r)) for r in rows]
...
def mostrar_businesses_user(user_id):
	conn = get_connection()
	with conn.cursor() as cur:
		cur.execute("SELECT id, nome, logo_path, added_at, by_user, premium FROM business WHERE by_user = %s", (user_id,))
		rows = cur.fetchall()
		colnames = [desc[0] for desc in cur.description]
		meusbusinesses = [dict(zip(colnames, r)) for r in rows]
	conn.close()
